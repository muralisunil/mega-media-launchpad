...
interface ThemeVars {
  ...
  "--hero-bg": { light: string; dark: string };
  gradient: string;
}
...
const ThemePicker = () => {
  ...
  const [active, setActive] = useState("Red (Current)");
  ...
  const applyTheme = (name: string) => {
    ...
    root.style.setProperty("--popover", t["--card"][mode]);
    ...
    const style =
      document.getElementById("theme-gradient-override") ||
      (() => {
  ...
  // Re-apply when light/dark mode toggles
  useEffect(() => {
